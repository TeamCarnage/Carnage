package xyz.carnage.manager.sound;
...
public class SoundManager {
    ...
    public static void playItemSound(String eventType, String soundName, World world, PlayerEntity player) {
        Identifier soundId = ITEM_SOUNDS.getOrDefault(soundName, new HashMap<>()).get(eventType);
    ...
    public static void playVanillaSound(SoundEvent soundEvent, PlayerEntity player, SoundCategory soundCategory, float soundVolume, float soundPitch) {
        player.getWorld().playSound(null, player.getBlockPos(), soundEvent, soundCategory, soundVolume, soundPitch);
    }

    public static void registerSounds(String modId) {
        // Register Sounds (not for the events).

        // Usage: registerSound(modId, "item ID - same as in CarnageItems")
